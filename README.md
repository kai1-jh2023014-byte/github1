...
AI を ON にすると、探索結果に従って通常の操作（移動・回転・ハードドロップ、必要ならホールド）で自動プレイします。既定の探索は Beam Search（深さ 3、幅 12）です。1-PLY / 2-PLY は従来のベースラインです。
...
AI Core は `TetrisGameState` を受け取り `TetrisAction` を返します。ブラウザ版は `BrowserGameAdapter` がそれを `GameEngine.input()` に変換します。探索は 1-ply / 2-ply / Beam を切り替えできます。Hold・T-spin・REN・B2B は Core の状態機械です。既定の Beam は測定上 Hold を使いません（Hold を探索候補にするとラインが落ちたため）。
